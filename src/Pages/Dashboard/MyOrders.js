...
const MyOrders = () => {
  ...
  const { data: myOrders, isLoading, refetch } = useQuery('myOrders', () => fetch(`https://assignment-12-server-orpin.vercel.app/myorders?email=${email}`).then(res => res.json()));
  ...
  const handleOrderDelete = id => {
    fetch(`https://assignment-12-server-orpin.vercel.app/myorders/${id}`, {
      method: 'DELETE',
      headers: {
